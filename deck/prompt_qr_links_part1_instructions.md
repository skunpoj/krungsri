# Prompt-QR links — PART 1 of 3: instructions & test cases

Split from a single `prompt_qr_links.md` that got too large to read in
one shot (mostly from long percent-encoded QR URLs). This part has all
narrative instructions: context, every test case's steps, results so
far, and the coverage matrix. The per-slide prompt text + QR URLs
referenced from here are in:
- `prompt_qr_links_part2_qr_howto1to3.md` (HOW TO 1, 2, 3)
- `prompt_qr_links_part3_qr_howto4to5_bonus.md` (HOW TO 4, 5, Bonus 1, Bonus 2)
